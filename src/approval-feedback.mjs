...
export function approvalFeedback(data, money, fallback) {
  ...
  switch (error.code) {
    case 'approval_per_request_limit':
      return 'This request needs up to ' + required + ', but your connection allows ' + available + ' per request. Increase the limit in Apiosk > Connections, or ask a smaller question.' + end;
    case 'approval_daily_limit':
      return 'This request needs up to ' + required + ', but your connection has ' + available + ' left in its daily budget. Increase the daily limit in Apiosk > Connections, try later, or ask a smaller question.' + end;
