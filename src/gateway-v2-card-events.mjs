...
function observeTask(data){
 ...
 const idle=!data.context_view?.worker_active&&['needs_input','cancelled','succeeded','partial','failed','unsupported'].includes(data.status);
 ...
 if(parsed.origin!=='__APIOSK_GATEWAY_ORIGIN__'||parsed.pathname!=='/v2/tasks/'+ref+'/events')return;
