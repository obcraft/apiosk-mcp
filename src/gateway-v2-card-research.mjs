...
renderResult=function(data){
 ...
 for(const result of results){if(cbs?.records.includes(result))continue;renderSingleResult({...data,result});const heading=sections.lastElementChild?.querySelector('h3');if(heading&&result?.subject?.label)heading.textContent='Result · '+formatCompanyText(result.subject.label,result)}
 ...
 if(report||analysis){const s=surface?.section||el('section','section result-extras');if(!surface)sections.append(s);const links=surface?.links||el('div','result-links');if(!surface)s.append(links);
  ...
  if(analysis&&!verdict){const details=surface?.details||el('details','compact-details result-details');if(!surface){details.append(el('summary','','Details'));details.ontoggle=()=>window.apiosk.resize();s.append(details)}const notes=el('div','analysis-notes');notes.append(el('p','key',cbs?'Toelichting':'Analysis'));
   for(const observation of analysis.observations||[]){notes.append(el('p','result',formatCompanyText(observation.text,data)));const evidence=el('details','evidence-details');evidence.append(el('summary','',cbs?'Onderbouwing':'Source evidence'));for(const item of observation.evidence||[])evidence.append(el('p','value',formatCompanyText(item.subject?.label||item.source?.name||'Source',data)+': '+formatSourceValue(item.value,item.field||item.pointer,null,data)+' · '+item.pointer));notes.append(evidence)}
   for(const limitation of analysis.limitations||[])(data.status==='partial'?s:notes).append(el('p','notice',formatCompanyText(limitation,data)));details.append(notes)
  }
 }
