...
For multiple companies, preserve each returned entity_ref and its known_facts separately in the same saved task. Never substitute the last company's KVK number for another company. The gateway quotes one combined cap and enforces the annual-accounts cooldown; show context_view.cooldown.until as a countdown and honor retry_after_ms. A cooldown poll makes no source purchase. Continue only gateway-issued actions under the existing exact quote approval. context_view.results contains every current source result, not just the last result. When analysis was requested, the backend OpenRouter model produces context_view.analysis from saved source evidence. Present that analysis and its limitations; do not buy additional data or silently replace missing filings with invented figures.
