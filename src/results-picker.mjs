...
<div class="eyebrow">Choose an API</div><h2 id="job">Apiosk</h2>
<div id="rows" class="rows"></div>
<form id="fields" class="fields"></form>
<div class="actions"><button id="run" class="primary" type="button">Run</button><button id="stop" class="secondary" type="button">Not now</button></div>
...
let options=[],picked=null,busy=false;
...
function pick(id){picked=options.find(o=>o.id===id)||null;
 ...
 run.textContent=picked?'Run '+picked.provider+' · '+picked.price_label:'Run';
 setStatus('Nothing is spent until you press run.');window.apiosk.resize()}
function drawRow(option,index){const row=document.createElement('label');row.className='row';row.dataset.id=option.id;
 ...
 mid.append(name);
 const bits=[];if(option.relevance!=null)bits.push(option.relevance+'/100 relevance');if(option.source)bits.push(option.source);
...
function render(output){const selection=output&&output.selection;options=(selection&&selection.options)||[];
...
run.addEventListener('click',async()=>{if(!picked||busy)return;let callArgs;
 ...
 try{
  if(window.apiosk.can.callTool){const result=await window.apiosk.callTool('apiosk_execute',callArgs);
   ...
   else if(result&&(result.status==='ok'||result.ok===true)){setStatus('Done. The result is in the conversation.','ok');
    await window.apiosk.say('Summarise the Apiosk result and answer my original question. Include the charged price briefly.')}
 ...
 catch(error){setStatus((error&&error.message)||'The call failed.','error')}
 finally{busy=false;if(!status.classList.contains('ok')){run.disabled=false;stop.disabled=false}}});
stop.addEventListener('click',async()=>{if(busy)return;run.disabled=true;stop.disabled=true;
