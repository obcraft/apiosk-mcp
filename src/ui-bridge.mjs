...
function rpc(method,params){return new Promise((resolve,reject)=>{const id=++rpcId;
 pending.set(id,{resolve,reject});send({jsonrpc:'2.0',id,method,params:params||{}});
 setTimeout(()=>{if(pending.has(id)){pending.delete(id);reject(new Error('timeout'))}},180000)})}
...
const api={
 ...
 can:{callTool:false,say:false,openLink:false},
 ...
 async say(text){
  ...
  if(mcp){try{await rpc('ui/message',{role:'user',content:{type:'text',text}});return true}catch(e){return false}}
  return false},
...
window.apiosk=api;
(async()=>{
 if(window.openai){api.can={callTool:!!window.openai.callTool,say:!!window.openai.sendFollowUpMessage,openLink:!!window.openai.openExternal};emit(openaiData())}
 if(window.parent===window)return;
 try{
  const result=await rpc('ui/initialize',{appCapabilities:{availableDisplayModes:['inline']}});
  mcp=true;host=(result&&result.hostCapabilities)||{};
  api.can={callTool:true,say:true,openLink:true};
