...
import { APIO_V2_CARD_URI, APIO_V2_CARD_LEGACY_URIS } from '../src/gateway-v2-card.mjs';
...
for (const host of ['chatgpt','claude']) test(`${host}: v2 initialize, tools and resource share the same contract`,async()=>{
 ...
 try {
  ...
  const {resources}=await client.listResources();assert.equal(resources.length,2+APIO_V2_CARD_LEGACY_URIS.length);
